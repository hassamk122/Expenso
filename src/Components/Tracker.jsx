...
function Tracker() {
    ...
    const AddRows = () => {
        if (Price && Itemname) {
            setRows([...rows, { Price, Itemname }]);
            TotalPrice.current = TotalPrice.current + parseInt(Price);
            setPrice('');
            setItemName('');
        }
    };
    const deleteRow = (index) => {
        TotalPrice.current = TotalPrice.current - rows[index].Price;
    ...
    const downloadTableAsText =()=>{
        ...
        rows.forEach((row)=>{
            ...
            textData = textData + `${i++}\t\t${row.Itemname}\t\t${row.Price}\n`;
        });
        ...
        <div className='flex items-center justify-center flex-col flex-wrap mt-8 sm:p-4 font-Montserrat'>
            <div className='flex font-normal text-secondary-blue dark:text-secondary-white border-[1px] border-primary-blue dark:border-secondary-white rounded-lg m-6 p-5 flex-col'>
                ...
                <div className=' flex mt-2 flex-col'>
                    <h1 className='text-lg font-semibold gap-1 flex text-left items-center flex-row pb-2'> <ShoppingBasket size={20} absoluteStrokeWidth={true} />Item :</h1>
                    <input type='text' className='focus:outline-primary-blue border-[1px] rounded-sm px-2  border-alternate-blue dark:border-secondary-white placeholder:p-2'
                        placeholder='Enter Item Name' value={Itemname} onChange={(e) => setItemName(e.target.value)}></input>
                </div>
                <div className=' flex mt-2 flex-col'>
                    <h1 className='text-lg font-semibold gap-1 flex text-left items-center flex-row pb-2'> <CircleDollarSign size = {20}/>Price :</h1>
                ...
                <div className=' flex mt-4 items-center justify-center '>
                    <button onClick={AddRows} className='bg-primary-blue dark:bg-primary-white text-primary-white dark:text-primary-black px-3 rounded-sm cursor-pointer'>Add</button>
                </div>
            </div>
                ...
                <table className="border-collapse border-[1px] border-secondary-blue text-alternate-blue dark:border-secondary-white font-Montserrat dark:text-secondary-white text-sm">
                    <thead>
                        <tr className='font-bold'>
                            <td className='border py-2 px-3 px text-center'>#</td>
                    ...
                    <tbody>
                        {rows.map((row, index) => (
                            <tr key={index}>
                                ...
                                <td className='border py-2  px-4 text-center'><button onClick={() => deleteRow(index)}
                                    className='cursor-pointer'
                                >
                                    <Trash2 size={20} absoluteStrokeWidth={true} className='text-primary-blue dark:text-secondary-white ' /></button></td>
                            </tr>
                        ))}
                        <tr className='font-semibold'>
                            ...
                            <td className='border py-2 px-4  text-center' ><button onClick={DeleteAllRows}><Delete/></button></td>
                        </tr>
                    </tbody>
                </table>
            </div>
            <div className='flex font-Montserrat items-center justify-center flex-col'>
                <div className='flex mt-4'>
                    ...
                    <input type='text' value={FileName} onChange={(e)=> setFileName(e.target.value)} className='focus:outline-none text-secondary-black  dark:text-secondary-white' placeholder='Enter file Name'/>
                ...
                border-2 border-lime-500 rounded-lg py-1 px-3 text-primary-white sm:text-lime-500  bg-lime-500 sm:bg-transparent sm:hover:bg-lime-500 sm:hover:text-primay-white transition delay-50 duration-300
                 hover:text-primary-white ease-in-out' onClick={downloadTableAsText}>
                    Download <ArrowDownToLine size={20}  absoluteStrokeWidth={true}/></button>
                    </div>
            </div>
        </div>
